...
Coordinate_st *FSMC_SRAM_PictureDeal1(u16 sx,u16 sy,u16 ex,u16 ey, u16 *newAddr,u16 *oldAddr)
{
 ...
 vu16 tmp_a,tmp_b,tmp_c = 0;
	...
	{
 		LCD_SetCursor(sx,sy+y_id);   	//设置光标位置 
		 LCD_WriteRAM_Prepare();     //开始写入GRAM
...
//						LCD->LCD_RAM = GREEN;
	...
	if(ArP.DspFocus)
	{
		...
		//冒泡排序 从大到小
		hotpoint_st temp={0};
		for(i=0;i<PICTURE_HEIGHT-1;i++)//n个元素，只需要n-1趟冒泡
		{
			for(j=1;j<PICTURE_HEIGHT-i;j++)//j=1,为了和前一个a[j-1]进行比较。n-i表示每趟排序都有一个元素被放置到后面正确的位置，之后不需移动。
			{
		...
		for(i=0;i<PICTURE_WIDHT-1;i++)//n个元素，只需要n-1趟冒泡
		{
			for(j=1;j<PICTURE_WIDHT-i;j++)//j=1,为了和前一个a[j-1]进行比较。n-i表示每趟排序都有一个元素被放置到后面正确的位置，之后不需移动。
			{
...
/*
		tmp_a = (unsigned int)(minX*0.2);//上限
		tmp_b = (unsigned int)(minX*0.8);//下限
		tmp_c = (unsigned int)(minX*0.6);//中间
*/
		tmp_a = 2;
		tmp_b = minX - 2;
		tmp_c = minX - 4;
